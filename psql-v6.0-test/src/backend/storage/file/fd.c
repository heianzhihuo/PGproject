...
/*Vfd 是双向链表的节点，*/
typedef struct vfd {
    signed short        fd;/*真实文件描述符，未打开则为-1*/
    unsigned short      fdstate;/*虚拟文件描述符的标记位，如果第0位是1，表明该文件内容已经被修改过，但没有被协会到磁盘，在关闭文件时要将该文件写入到磁盘；如果第0位是0，则表明该文件在被关闭时要被删除*/

#define FD_DIRTY        (1 << 0)

    File        nextFree;/*指向下一个空闲的VFD*/
    File        lruMoreRecently;/*最近更长使用*/
    File        lruLessRecently;/*最近更不常使用*/
    long        seekPos;/*当前读写指针位置*/
    char        *fileName;/*文件名，空闲VFD为空*/
    int         fileFlags;/*打开文件时的标志，只读，只写，读写*/
    int         fileMode;/**/
} Vfd;
